	...
	var defaults = {
		...
		files: {
			...
			modules: {
				...
				terms: '{url:first}/{remotePath}/files/{language}/terms.html?v=4',
				...
				codepopup: '{url:first}/{remotePath}/files/{language}/code.html',
	...
	$.extend(createObjectWithKey(plugin.name, function (options) {
		...
		function moduleFistInclude() {
			...
			$.when(
				...
				$.when(moduleExitPopup(), moduleWaitPopup(), moduleCodePopup()).done(function () {
		...
		function moduleInputBinding() {
			...
			$.each(general.$form, function () {
				...
				$(phone, form).on(general.inputEvent, function () {
					...
					$phone.val($(this).val());
					if ([0, 1].includes(general.Config.phoneValidation)) {
			...
			ajaxCatchUp(general.catch.phone).done(function (response) {
				var data = JSON.parse(response);
				if (data.success) {
					validation($phone);
				} else {
					general.Form.invalidInput($phone, 'phone');
				}
			}).fail(function () {
				general.Form.invalidInput($phone, 'phone');
			});
			...
			if (!general.Form.smsVerification) {
			...
			if (
				...
				general.promise.config.done(function () {
					(function checkConfigTrackers(isDevCallback, isProdCallback) {
						var prodConfigKeywords = ['fbq', 'lead', 'pixel', 'matomo', 'piwik', 'ga', 'ym', 'ttq', 'bge'];
		...
		/**
		 * @module
		 * Модуль инициализации Code popup
		 * @returns {object} - $.Deferred() объект, promise
		 **/
		function moduleCodePopup() {

			var initCodepopup = function (html) {
				if (html) general.$body.append(html);
				return general.promise.codepopup.resolve();
			};

			if (verifyHTML(general.selector.codepopup)) {
				initCodepopup();
			} else {
				ajax(setting.files.modules.codepopup).done(initCodepopup)
			}
		...
		function moduleEventExit() {
			var exitpopup = {
				...
				//При нажатии на кнопку Назад следует показ окна, без перехода на предидущую страницу (только первый раз)
				onhashchange: true,
			...
			if (exitpopup.onhashchange) {
				window.history.pushState({
					link: window.location.href
				}, null, window.location.href)
				window.onpopstate = function () {
					exitpopup.methods.open();
				};
			}
